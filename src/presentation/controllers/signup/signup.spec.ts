...
describe('SignUp Controller', () => {
  ...
  test('Should return for 400 if no passwordConfirmation fails', () => {
    const { sut } = makeSut();
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Should return for 400 if an invalid email is provided', () => {
  ...
  test('Should return for 200 if valid data is provided', () => {
    const { sut } = makeSut();
    const httpRequest = {
      body: {
        name: 'valid_name',
